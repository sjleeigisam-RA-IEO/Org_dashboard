...
const migrationPath = path.join(
  ...
  '01. RA Portal',
  'migrations',
  '2026-08-14_party_exposure_fact_cache.sql',
);
...
const sql = apply
  ...
  : migrationSql.replace(/commit;\s*$/i, `
      select
        (select count(*)::bigint from ra_internal.party_exposure_fact_source) as source_rows,
        (select count(*)::bigint from ra_internal.party_exposure_fact_cache) as cache_rows,
        (select count(*)::bigint from public.party_exposure_fact) as public_fact_rows,
        (select count(*)::bigint from public.party_exposure_current) as current_rows;
      rollback;
    `);
...
await fs.writeFile(
  path.join(outputDir, apply ? 'cache_apply_result.json' : 'cache_dry_run_result.json'),
