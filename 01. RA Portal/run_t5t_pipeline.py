...
def main():
    ...
    if not args.skip_sync:
        run_step(
            ...
            append_date_args(["01. RA Portal/sync_notion_raw_to_sql.py"], args.date_from, args.date_to),
        )

    run_step(
        "2. Resolve missing T5T writer identities",
        ["01. RA Portal/backfill_t5t_writer_ids.py", "--apply", "--require-complete"],
    )
    run_step(
        "3. Apply alias/project/mission rules to unmatched rows",
        append_date_args(
            ["01. RA Portal/apply_t5t_manual_aliases.py", "--apply", "--sample-size", "0"],
            args.date_from,
            args.date_to,
        ),
    )
    run_step(
        ...
        append_date_args(
            [
                "01. RA Portal/apply_t5t_manual_aliases.py",
                "--status",
                "general_work",
                "--apply",
                "--sample-size",
    ...
    run_step(
        ...
        append_date_args(["01. RA Portal/classify_t5t_general_work.py", "--apply", "--sample-size", "0"], args.date_from, args.date_to),
    )
    run_step(
        "6. Match remaining items against project/fund/asset masters",
        append_date_args(["01. RA Portal/match_t5t_entities.py", "--apply", "--sample-size", "0"], args.date_from, args.date_to),
    )
    run_step(
        "7. Normalize dashboard tables",
        append_date_args(["01. RA Portal/normalize_t5t_logs.py"], args.date_from, args.date_to),
    )
