...
function createHandler({ rpc = shared.rpc } = {}) {
  return async function handler(req, res) {
    ...
    try {
      ...
      // All CRM writes are temporarily locked: even a conflict/replay response
      // could otherwise reveal the original private record. RM APIs are separate.
      return auth.json(res, 403, privacy.mutationDenied());
    } catch (error) {
