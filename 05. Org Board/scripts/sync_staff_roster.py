...
DASHBOARD_DIR = ROOT / "05. Org Board"
DEFAULT_SOURCE_CANDIDATES = sorted(ROOT.parent.glob("03.*/*/*/*260701*.xlsx"))
DEFAULT_SOURCE = DEFAULT_SOURCE_CANDIDATES[0] if DEFAULT_SOURCE_CANDIDATES else ROOT.parent / "staff_roster_260701_not_found.xlsx"
...
class SupabaseRest:
    ...
    def delete_ids(self, name: str, column: str, ids: list[str], chunk_size: int = 40) -> None:
        for index in range(0, len(ids), chunk_size):
            ...
            encoded_filter = urllib.parse.quote(encoded, safe='(),"')
            self.request("DELETE", f"{name}?{column}=in.({encoded_filter})")
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "05. Org Board" / "outputs" / "org_roster_sync_20260715")
