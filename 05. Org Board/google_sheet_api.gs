...
function doGet(e) {
  ...
  const seatSheetName = (e && e.parameter && e.parameter.seat_sheet) || SHEET_NAMES.SEAT_LAYOUT;
  const seatSheetNames = parseSeatSheetNames_((e && e.parameter && e.parameter.seat_sheets) || seatSheetName);
  ...
  if (action === "upsertSeatLayout") {
    const result = upsertSeatLayoutRow_(ss, e.parameter || {}, seatSheetName);
  ...
  const seatLayoutRows = readSeatLayoutRows_(ss, seatSheetNames, assignments);

  const payload = buildDashboardPayload_(assignments, sectionsMeta, seatLayoutRows, seatSheetNames);
...
function buildDashboardPayload_(assignmentRows, sectionRows, seatLayoutRows, seatSheetNames) {
  ...
  return {
    ...
    sections,
    seatLayout: buildSeatLayoutPayload_(seatLayoutRows, seatSheetNames),
  };
}

function buildSeatLayoutPayload_(rows, seatSheetNames) {
  return {
    sheetName: (seatSheetNames || []).join(",") || SHEET_NAMES.SEAT_LAYOUT,
    updatedAt: Utilities.formatDate(new Date(), Session.getScriptTimeZone(), "yyyy-MM-dd HH:mm:ss"),
    rows: rows || [],
  };
}

function normalizeSeatLayoutRows_(rows, sheetName, assignmentOrgMap) {
  return rows.map((row) => ({
    ...
    team_org_id_seat: String(row.team_org_id_seat || "").trim() || inferSeatTeamOrgId_(row, assignmentOrgMap),
...
function parseSeatSheetNames_(value) {
  return String(value || "")
    .split(",")
...
function buildAssignmentOrgMap_(assignmentRows) {
  const map = {};
  assignmentRows.forEach((row) => {
    const personName = String(row.person_name || "").trim();
    if (!personName || map[personName]) {
      return;
    }
    map[personName] = String(row.team_org_id || row.part_org_id || row.group_org_id || "").trim();
  });
  return map;
}

function inferSeatTeamOrgId_(row, assignmentOrgMap) {
  const personName = String(row.person_name || "").trim();
  if (!personName) {
    return "";
  }
  return String(assignmentOrgMap[personName] || "").trim();
}

function readSeatLayoutRows_(ss, seatSheetNames, assignmentRows) {
  const orgMap = buildAssignmentOrgMap_(assignmentRows || []);
  const rows = [];
  (seatSheetNames || []).forEach((sheetName) => {
    const sheet = getOptionalSheet_(ss, sheetName);
    if (!sheet) {
      return;
    }
    rows.push.apply(rows, normalizeSeatLayoutRows_(readSheetObjects_(sheet), sheetName, orgMap));
  });
  return rows;
}

function upsertSeatLayoutRow_(ss, params, seatSheetName) {
  const sheet = getRequiredSheet_(ss, seatSheetName || SHEET_NAMES.SEAT_LAYOUT);
