...
| `05. Org Board/` | 조직/좌석 대시보드. 현재 Google Sheet/WebApp 및 로컬 JSON 기반 | 당분간 기존 운영 유지. DB seed는 이관 준비 |
| `01. RA Portal/portfolio-analysis/` | 펀드, 자산, AUM, 익스포저 분석 화면 | 핵심 자산 조회 화면 후보 |
| `02. T5T Board/` | T5T 업무 대시보드. 현재 Supabase v2 실험 화면으로 전환 중 | SQL 전환 검증 후 정식 화면화 |
| `pilot_form/` | IOTA Seoul 파일럿 워크플랫폼 UI | 같은 DB를 쓰는 파일럿. 스키마 정리 필요 |
| `01. RA Portal/migrations/` | Supabase migration SQL | 실제 DB 기준과 대조 관리 필요 |
| `01. RA Portal/supabase_seed/` | org/staff/seat/AUM/T5T seed 산출물 | 재현 가능한 이관 산출물 |
...
1. Excel/CRM 원천
2. `01. RA Portal/replace_supabase_*.py`
3. Supabase `funds`, `fund_assets`, `lender_exposures`, `beneficiary_exposures`
4. `01. RA Portal/portfolio-analysis/`
...
2. `05. Org Board/` 화면
3. `01. RA Portal/build_supabase_seed.py`
...
2. `01. RA Portal/process_t5t_csv.py` 또는 `01. RA Portal/sync_notion_raw_to_sql.py`
3. Supabase `t5t_form_submissions`, `t5t_form_items`
4. `02. T5T Board/t5t-service-v2.js`
5. `02. T5T Board/app-v2.js`
...
- `02. T5T Board/supabase-config.js`
- `02. T5T Board/tally api key.txt`
- `01. RA Portal/_archive/supabase sql pw.txt`
...
- `01. RA Portal/portfolio-analysis/index.html`
...
`02. T5T Board/index.html`에 `<!DOCTYPE html>`, `<html>`, `<head>`가 중복 삽입되어 있다. 브라우저가 보정할 수는 있으나 구조적으로 위험하다.
...
### P1. `01. RA Portal/.env` 기준을 기대하는 스크립트가 남아 있음 - 정리 완료

루트 `.env` 통합을 했지만 일부 스크립트는 `01. RA Portal/.env`를 읽는다.

- `01. RA Portal/setup_pilot_projects.py`
- `01. RA Portal/backfill_pilot_from_csv.py`
- `01. RA Portal/upsert_supabase_seed.py`

권장: 모든 Python 스크립트는 루트 `.env`를 우선 읽고, `01. RA Portal/.env`는 fallback으로만 둔다.

조치: `01. RA Portal/env_utils.py`를 추가하고 주요 로컬/DB 파이프라인 스크립트가 루트 `.env`를 우선 읽도록 정리했다. 브라우저에서 `.env`를 fetch하는 P0 항목은 이번 범위에서 건드리지 않았다.
...
2. T5T HTML 구조 수정
   - `02. T5T Board/index.html` 중복 doctype/head 제거
...
| 제외 파일군 | 이유 |
|---|---|
| `01. RA Portal/scratch/` | 1회성 진단, 보정, 검증, 패치 스크립트 묶음 |
| `scratch/` | 루트 단위 임시 점검 파일 |
| `01. RA Portal/_archive/` | 과거 자료, 캐시, 민감 백업, 외부 가이드 등 운영 기준 외 파일 |
| `crm dashboard/` | 통합 전 구형 CRM workspace. 현재 운영 진입점은 `01. RA Portal/portfolio-analysis/` |
| `automation_runtime/logs/`, `automation_runtime/state/` | 실행 로그와 상태값. 코드가 아니라 런타임 산출물 |
| `02. T5T Board/*.log`, `02. T5T Board/test_*.json`, `02. T5T Board/scratch_*.txt`, `02. T5T Board/debug_ids.py` | 로컬 디버그 산출물 |
| `02. T5T Board/tally api key.txt` | 민감 키 파일. Git 추적 대상에서 제외 |
| 루트 `notion_agent.py`, `notion_pages.txt`, `query_*` | 현재 통합 대시보드 기준 밖의 과거 Notion 조회 보조 스크립트 |
| `01. RA Portal/*_output.txt`, `01. RA Portal/org_report.txt`, `01. RA Portal/inspect_results.json` 등 | 로컬 실행 결과물 |
| `01. RA Portal/MODEL_WORK_ASSIGNMENT.md`, `01. RA Portal/NEXT_WORK_PLAN.md`, `01. RA Portal/task*.md`, `01. RA Portal/walkthrough.md`, `handoff_dashboard_status.md`, `05. Org Board/HANDOFF_2026-04-08.md` | 이전 작업 지시/인수인계 문서. 현재 기준 문서로 대체 |

계속 Git에 남기는 기준 파일은 다음과 같다.

- 통합 기준 문서: `INTEGRATED_DASHBOARD_ARCHITECTURE_AUDIT_2026-05-02.md`
- DB 기준 문서: `01. RA Portal/SUPABASE_MIGRATION_PLAN.md`, `01. RA Portal/MIGRATION_STATUS_2026-05-01.md`, `01. RA Portal/STAFF_ORG_KEY_POLICY.md`
- 실행 파이프라인: `01. RA Portal/build_*`, `01. RA Portal/process_t5t_csv.py`, `01. RA Portal/sync_notion_raw_to_sql.py`, `01. RA Portal/upsert_supabase_seed.py`, `01. RA Portal/replace_supabase_*.py`
- 화면 진입점과 정적 assets: `index.html`, `05. Org Board/`, `02. T5T Board/`, `01. RA Portal/portfolio-analysis/`, `pilot_form/`
