...
- `t5t-dashboard`: SQL 전환 테스트 중. 현재 화면 v2는 `t5t_form_items`를 조회하고, 최종 정규화 테이블(`t5t_logs`) 전환은 다음 단계로 둔다.
...
- `migrations/2026-05-01_iota_seoul_schema.sql`
  - 파일럿 워크플랫폼용 `public.iota_seoul_*` 테이블
  - 별도 `iota_seoul` schema가 아니라 Supabase JS에서 접근 가능한 `public` schema를 기준으로 한다.
...
파일럿 프로젝트 4건과 Notion 프로젝트 seed 619건은 `project_id`가 충돌하지 않는다. 파일럿은 `iota-*` prefix, Notion 프로젝트는 `project_notion_*` prefix를 사용한다. 프로젝트 master만 먼저 확장할 때는 아래 명령을 사용한다.

```powershell
python CRM_base\upsert_supabase_seed.py --table projects --table project_staff_links
```

2026-05-02 기준 이 명령으로 실제 DB는 `projects` 623건, `project_staff_links` 1,942건 상태가 되었다.
