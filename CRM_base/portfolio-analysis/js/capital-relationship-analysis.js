(function () {
  ...
  var FILTER_CONFIG = {
    ...
    minimumAmount: { label: '최소금액', property: 'currentAmount', type: 'amount' }
  };
  ...
  function normalizePartyRow(row, index, sourceKind) {
    ...
    var reviewStatuses = combinedArray(row, ['review_statuses'], [
      ...
      role === 'lender' ? 'lender_review_status' : 'beneficiary_cat_review_status'
    ...
    var normalized = {
      ...
      qualityFlags: combinedArray(row, ['quality_flags'], ['quality_flag', 'relationship_quality']),
      sourceIndex: index
    };
  ...
  function mergeFactRows(target, source) {
    [
      ...
      'projectNames', 'aliasNames', 'qualityFlags'
  ...
  function dedupeFactRows(rows) {
    ...
    rows.forEach(function (row) {
      ...
      var key = row.role + '|' + row.exposureId;
      if (!explicit.has(key)) {
    ...
    state.duplicateFactsSuppressed = suppressed;
    return output;
  }
  ...
  function aggregatePartyRows(rows) {
    var groups = new Map();
    rows.forEach(function (row) {
      var normalizedName = searchToken(row.partyName);
      var key = row.role + '|' + (normalizedName || row.partyId);
      if (!groups.has(key)) {
        groups.set(key, {
          resultId: key,
          role: row.role,
      ...
      group.partyIds = unique(group.partyIds.concat([row.partyId]));
  ...
  async function loadCapitalRelationshipData(force) {
    ...
    state.loadPromise = Promise.all([
      ...
      var facetResponse = responses[2];
      state.loadErrors = responses.filter(function (response) { return response.error; }).map(function (response) {
        return response.error.message;
      });
      state.facets = facetResponse.rows || [];
      state.facts = (factResponse.rows || []).map(function (row, index) {
        return normalizePartyRow(row, index, 'fact');
      });
      state.rankings = (rankingResponse.rows || []).map(function (row, index) {
        return normalizePartyRow(row, index, 'ranking');
      });
      ...
      if (state.facts.length > 0) {
        ...
        state.sourceLabel = '기관 집계 View';
        state.snapshotDate = maxSnapshotDate(state.rankings);
        state.duplicateFactsSuppressed = 0;
      } else {
        state.results = [];
        state.source = factResponse.error ? 'unavailable' : 'fact';
        state.sourceLabel = factResponse.error ? '조회 불가' : '현재 Fact';
        state.snapshotDate = '';
      }
      state.loaded = true;
      state.loading = false;
  ...
  function facetValuesFor(key) {
    ...
    return unique(values).sort(function (a, b) { return a.localeCompare(b, 'ko'); });
  }
  ...
  function matchesFilters(row) {
    ...
    if (filters.minimumAmount && row.currentAmount < numberValue(filters.minimumAmount) * MILLION) return false;
    return true;
  }
  ...
  function applyCapitalFilters(options) {
    ...
    state.filtered = state.results.filter(matchesFilters).sort(function (a, b) {
      return b.currentAmount - a.currentAmount || b.committedAmount - a.committedAmount || a.partyName.localeCompare(b.partyName, 'ko');
    });
  ...
  function classificationSubtotals(rows) {
    ...
    return Array.from(groups.values()).sort(function (a, b) { return b.current - a.current; });
  }
  ...
  function renderKpis(totals) {
    ...
    return [
      ...
      '<div><span>' + escapeHtml(role.committedLabel) + '</span><strong>' + escapeHtml(formatCompactWon(totals.committed)) + '</strong><small>' + escapeHtml(formatMillion(totals.committed)) + '백만원</small></div>',
      '<div class="is-primary"><span>' + escapeHtml(role.currentLabel) + '</span><strong>' + escapeHtml(formatCompactWon(totals.current)) + '</strong><small>' + escapeHtml(formatMillion(totals.current)) + '백만원</small></div>',
  ...
  function renderRankingTable() {
    ...
    return [
      ...
      '<div><h3>금액순 결과</h3><p>' + escapeHtml(role.currentLabel) + ' 기준 · 금액 단위 백만원</p></div>',
  ...
  function renderSubtotals(subtotals, reconciliation) {
    ...
    var rows = subtotals.length ? subtotals.map(function (row) {
      var share = reconciliation.total.current ? row.current / reconciliation.total.current * 100 : 0;
      return [
        '<tr>',
        '<td><strong>' + escapeHtml(row.label) + '</strong><span>' + formatInteger(row.parties) + '개</span></td>',
        '<td>' + escapeHtml(formatMillion(row.current)) + '</td>',
        '<td>' + share.toLocaleString('ko-KR', { maximumFractionDigits: 1 }) + '%</td>',
        '</tr>'
      ].join('');
    }).join('') : '<tr><td colspan="3" class="capital-table-empty">부분합이 없습니다.</td></tr>';
    var difference = Math.abs(reconciliation.differences.current);
    return [
      '<aside class="capital-subtotal-section">',
      '<div class="capital-section-heading">',
      '<div><h3>분류별 부분합</h3><p>' + escapeHtml(role.currentLabel) + ' 기준</p></div>',
  ...
  function factsForParty(row) {
    ...
    return state.facts.filter(function (fact) {
      return fact.role === row.role && searchToken(fact.partyName) === searchToken(row.partyName);
  ...
  function renderCapitalResults() {
    ...
    var fallbackWarning = state.source === 'ranking'
      ? '<span class="capital-source-warning">원천 Fact 대신 집계 View 사용</span>'
      : '';
