...
### [Rule 2] 멀티 클래스 운용 구조 및 AUM 무결성 보장 규칙 (Multi-Class Portfolio Consolidation)
*   **대상:** 멀티 클래스(Multi-class) 및 모자 구조 펀드의 운용 자본금 중복 계산 방지 및 정확한 통계 보장.
*   **명칭 기반 자동 맵핑 파서:**
    *   펀드명에 `"모투자"`, `"모부동산"`, 또는 클래스명이 없고 명칭 끝에 `"(운용)"` 포함 ➔ **`모펀드`** 분류
    *   펀드명에 `"자투자"`, `"자부동산"`, 또는 클래스명 `"(1종)"`, `"(2종)"`, `"(A종)"` 등 포함 ➔ **`자펀드`** 분류
    *   펀드명에 `"프로젝트"`, `"PFV"`, `"피에프브이"` 포함 ➔ **`프로젝트펀드`** 분류
    *   그 외 일반 단독 구조 ➔ **`독립펀드`** 분류
*   **AUM 집계 및 무결성 대원칙 (AUM Integrity Principle):**
    > [!IMPORTANT]
    > **수익증권 분할 및 자펀드 AUM '0'화 보정 대칙:**
    > 이지스 RA CRM의 원천 원장 데이터 구조상, **실질적인 총 운용자산(AUM, Equity, Loan, Deposit)의 값은 대표 운용 격판인 "모펀드((운용) 레코드)"에 100% 반영되어 저장**되어 있으며, 분할 수익증권 격판인 "자펀드((1종), (2종) 레코드)"의 AUM 값은 원천적으로 **`NULL` 또는 `0`원**으로 기재되어 있습니다.
    > 
    > 따라서 대시보드 및 시스템이 포트폴리오 운용 규모(AUM)를 계산할 때는 **절대 "모펀드"를 집계에서 필터링해 배제해서는 안 됩니다.** 만약 "모펀드"를 제외할 경우, 센터필드(2.2조원)를 비롯한 대형 핵심 자산 운용 금액 약 29.8조원이 통계에서 증발하는 대참사가 발생합니다.
    >
    > 자펀드들의 금융 규모 값이 0원이기 때문에, **전체 레코드를 필터링 없이 그대로 합산(`SUM`)하여도 이중 카운팅(Double Counting)이 전혀 발생하지 않으며** 공식 AUM 워크북과 일치하는 **71.14조원**의 순수 포트폴리오 운용액을 완벽하게 산출합니다.
    ```sql
    -- 올바른 AUM 집계 쿼리 (71.14조원 단일 진실 원천값 추출)
    SELECT SUM(benchmark_aum) 
    FROM funds;
    ```
