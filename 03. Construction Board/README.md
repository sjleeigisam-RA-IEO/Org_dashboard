...
- OpenDART awards: refreshed 2026-09-14 from `.env:DART_API_KEY`, 2021-09-14 to 2026-09-14, max 5 per company.
- OpenDART strategy disclosures: refreshed 2026-09-14 from `.env:DART_API_KEY`; investment, equity/capex, M&A/restructuring, financing, and related-party signals, max 5 per company. Category labels are retained only for filtering/dedupe metadata, not as visible article-title prefixes.
- Google News RSS: refreshed 2026-09-14, 365-day window, max 5 per company.
- Nara/G2B contracts: refreshed 2026-09-14 with a narrow 14-day query window because longer API windows can time out or exceed input limits. The Data.go.kr key is read from `.env:DATA_GO_KR_KEY`; key values must never be printed or written to logs.
