...
대상 경로: `03. Construction Board/`

현재 공개 URL: `https://sjleeigisam-ra-ieo.github.io/Org_dashboard/03.%20Construction%20Board/`
...
- `HANDOFF.md`: 이 문서.
- `data/03. Construction Board_data.json`: 순위표와 각 행의 수주/기사/신용등급이 합쳐진 렌더링용 스냅샷.
...
온라인 코멘트는 `01. RA Portal/portfolio-analysis/config.js`의 Supabase 공개 URL/키를 읽어 정적 HTML 안에 반영한다. 이 키는 브라우저에서 쓰는 publishable key이며, 운영 시에는 Supabase RLS 정책을 반드시 유지해야 한다.
...
$backup = Join-Path $env:TEMP "construction_refresh_before_$stamp"
New-Item -ItemType Directory -Path $backup -Force | Out-Null
Get-ChildItem ".\03. Construction Board\data" -Filter 'construction_*cache.json' | ForEach-Object {
  Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $backup $_.Name) -Force
}

# OpenDART 단일판매ㆍ공급계약체결 수주 캐시
python ".\03. Construction Board\scripts\update_construction_dart_awards.py" --lookback-years 5 --per-company 5

# 나라장터 공사 계약정보 캐시
$today = Get-Date
$naraStart = $today.AddDays(-14).ToString('yyyyMMdd')
$naraEnd = $today.ToString('yyyyMMdd')
python ".\03. Construction Board\scripts\update_construction_nara_contracts.py" --start-date $naraStart --end-date $naraEnd --per-company 5 --max-pages 5 --rows-per-page 50

# Google News RSS 기사 캐시
python ".\03. Construction Board\scripts\update_construction_company_news.py" --days 365 --per-company 5

# OpenDART 투자/전략 공시 캐시
python ".\03. Construction Board\scripts\update_construction_dart_strategy.py" --lookback-years 5 --per-company 5
...
# 순위 원천 수집 + 캐시 결합 + index.html/data JSON 직접 생성
python ".\03. Construction Board\scripts\build_construction_source_status.py"

# 비교용 임시 백업 제거
Remove-Item $backup -Recurse -Force
```

각 수집 스크립트는 `03. Construction Board/data/`에 캐시를 직접 갱신한다. 빌더는 배포 파일인 `03. Construction Board/index.html`과 `03. Construction Board/data/construction_source_status_data.json`을 직접 생성한다. 루트 `outputs/`를 만들거나 별도 복사하는 단계는 없다.
...
자동화가 실행할 때는 기존 무관 dirty 파일을 건드리지 않고, `03. Construction Board/` 관련 파일만 stage/commit/push한다. 원격 배포 확인은 항상 포함한다. Pages 확인 뒤에는 같은 run log entry에 `commit`, `pushed`, `pages_verified`, `pages_verified_at`, `pages_url`을 최종 반영하고, 이 로그 변경만 log-only commit/push로 한 번 더 올릴 수 있다.

향후 GitHub Actions로 이전할 수는 있지만, 그 경우에도 현재 run log 계약과 화요일 fallback 조건을 유지해야 한다. Actions secrets에는 `OPENDART_KEY` 또는 `DART_API_KEY`, `DATA_GO_KR_KEY`만 넣고, 관리자성 Supabase key는 정적 HTML이나 공개 로그에 노출하지 않는다.
...
```powershell
python -m py_compile ".\03. Construction Board\scripts\build_construction_source_status.py"
python -m py_compile ".\03. Construction Board\scripts\update_construction_company_news.py"
python -m py_compile ".\03. Construction Board\scripts\update_construction_credit_ratings.py"
...
```text
https://sjleeigisam-ra-ieo.github.io/Org_dashboard/03.%20Construction%20Board/?v=YYYYMMDDHHMM
```
