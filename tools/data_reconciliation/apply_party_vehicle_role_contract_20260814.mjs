...
const migrationPath = path.join(
  ...
  "CRM_base",
  "migrations",
  "2026-08-14_party_vehicle_role_contract.sql",
);
...
const sql = apply
  ...
  : migrationSql.replace(/commit;\s*$/i, `
      select * from public.party_vehicle_role_contract_audit;
      rollback;
    `);
...
const verification = apply
  ? await query(`
      ...
      select
        role_class, capital_scope, include_in_external_investor_rollup,
