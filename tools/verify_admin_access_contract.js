...
vm.runInContext(
  fs.readFileSync(path.join(ROOT, 'CRM_base', 'portfolio-analysis', 'js', 'asset-canonical.js'), 'utf8'),
  nonAdminContext,
...
const sourceChecks = [
  ...
  ['org_dashboard/admin.html', /!window\.RAAuth\?\.isAdminUser\?\.\(user\)/],
  ['CRM_base/portfolio-analysis/js/asset-canonical.js', /function renderAdminBar\(container\) \{\s+if \(!isAuthorizedAdmin\(\)\) return;/s],
  ['org_dashboard/seat-layout.js', /\$\{authorizedAdmin \? `<button class="seat-admin-btn/s],
  ['org_dashboard/seat-layout.js', /async function saveAdminChanges\(\) \{\s+if \(!isAuthorizedAdmin\(\)\)/s],
];
