...
parser = argparse.ArgumentParser(description="Run resumable year-range MOLIT Seoul non-residential trade backfill")
...
for lawd_cd, district_name in districts.items():
    for start_date, end_date in windows:
        ...
        result = ingest_partition(
            ...
            runner_version=f"backfill-{args.year}.1",
        )
        row = {
...
output = Path(f"artifacts/{campaign_slug}-molit-seoul-summary.json")
