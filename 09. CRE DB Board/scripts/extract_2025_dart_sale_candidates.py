...
PIPELINE = "DART_TYPE_ASSET_SALE_RULE_V2"
...
def main() -> None:
    ...
    try:
        ...
        for version_id, document_id, receipt_no, url, title, published_at, stored_text, metadata_json in rows:
            ...
            cur = con.execute(
                """INSERT OR IGNORE INTO event_mentions(
                   ...
                   VALUES (?,?, 'dart-type-asset-sale:v1',?,?,?,?,0.85,'REVIEW_READY')""",
        ...
        con.close()
    output = {"scannedFullTextFilings":len(rows),"candidateCount":len(candidates),"inserted":inserted,"candidates":candidates}
    path = ROOT / "artifacts" / "opendart-2025-type-asset-sale-candidates.json"
