...
def main() -> None:
    ...
    range_start = f"{args.year:04d}-{args.start_month:02d}-01"
    end_year, end_month = (args.year + 1, 1) if args.end_month == 12 else (args.year, args.end_month + 1)
        ...
        """SELECT d.external_document_key,d.canonical_url,v.title,v.published_at,
           ...
           WHERE s.source_code='OPENDART'
             ...
             AND (v.title LIKE '%유형자산양도%' OR v.title LIKE '%유형자산양수%'
                  OR v.title LIKE '%유형자산취득%' OR v.title LIKE '%영업양도%'
                  OR v.title LIKE '%영업양수%')
    ...
    for month, docs in sorted(by_month.items()):
        ...
        result = ingest_partition(
            ...
            job_code=f"{campaign_code}_OPENDART_SALE_DOCUMENT_TEXT_V3",
            category_code="SALE", window_start=start, window_end=end,
            query_rendered=f"OpenDART document.xml full text V3 for {args.year} M{args.start_month:02d}-M{args.end_month:02d} 유형자산양도/양수/취득 및 영업양도/양수 filings",
            documents=docs, runner_version=f"backfill-{args.year}-dart-document-text-v3",
        )
    ...
    output = ROOT / "artifacts" / f"{campaign_slug}-opendart-sale-document-text-v3-summary.json"
