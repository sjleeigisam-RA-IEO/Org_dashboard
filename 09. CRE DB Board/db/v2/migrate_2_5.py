"""Apply the V2.5 institutional LP mandate migration after a consistent SQLite backup."""
...
MIGRATION = ROOT / "migrations" / "2.5.0_lp_mandates.sql"
EXPECTED_FROM = "2.4.0"
EXPECTED_TO = "2.5.0"
REQUIRED_TABLES = {"lp_mandates", "lp_mandate_tracks", "lp_mandate_guidelines", "lp_mandate_selections", "lp_mandate_selection_members", "lp_mandate_selection_vehicles", "lp_mandate_amounts", "lp_mandate_deployments"}
REQUIRED_VIEWS = {"v_lp_mandate_source_balance", "v_lp_mandate_deal_sources"}
...
def main() -> None:
    ...
    backup_path = (args.backup or db_path.parent.parent / "backups" / f"market-pre-v2.5.0-{stamp}.db").resolve()
    ...
    try:
        ...
        required_stage_count = con.execute(
            "SELECT COUNT(*) FROM event_stages WHERE stage_code IN ('MANAGER_RFP_OPEN','MANAGER_SELECTED')"
        ).fetchone()[0]
        required_predicate_count = con.execute(
            "SELECT COUNT(*) FROM predicate_definitions WHERE predicate_code LIKE 'LP_MANDATE_%'"
        ).fetchone()[0]
        integrity = con.execute("PRAGMA integrity_check").fetchone()[0]
        fk = con.execute("PRAGMA foreign_key_check").fetchall()
        if (actual != EXPECTED_TO or missing or required_stage_count != 2
                or required_predicate_count != 8 or integrity != "ok" or fk):
            raise RuntimeError(
                f"migration validation failed: version={actual}, missing={missing}, "
                f"stages={required_stage_count}, predicates={required_predicate_count}, "
