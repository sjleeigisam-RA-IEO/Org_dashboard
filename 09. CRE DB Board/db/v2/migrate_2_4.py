"""Apply the V2.4 sale-process relation migration after a consistent SQLite backup."""
...
MIGRATION = ROOT / "migrations" / "2.4.0_sale_process_relations.sql"
EXPECTED_FROM = "2.3.0"
EXPECTED_TO = "2.4.0"
REQUIRED_TABLES = {"sale_process_relations"}
REQUIRED_VIEWS = {"v_sale_process_relations"}
...
def main() -> None:
    ...
    backup_path = (args.backup or db_path.parent.parent / "backups" / f"market-pre-v2.4.0-{stamp}.db").resolve()
    ...
    try:
        ...
        if actual != EXPECTED_TO or missing or integrity != "ok" or fk:
            raise RuntimeError(f"migration validation failed: version={actual}, missing={missing}, integrity={integrity}, fk={len(fk)}")
        con.commit()
    finally: con.close()
