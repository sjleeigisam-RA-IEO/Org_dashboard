...
MIGRATION = ROOT / "migrations" / "2.8.0_document_enrichments.sqlite.sql"
EXPECTED_FROM = "2.7.0"
EXPECTED_TO = "2.8.0"
EXPECTED_COLUMNS = (
    "document_enrichment_id", "document_version_id", "enrichment_kind", "pipeline_version",
    "source_content_sha256", "resolved_url", "content_mode", "summary_method", "summary_text",
    "safe_excerpt", "parser_name", "parser_version", "fetched_at", "generated_at", "status_code",
    "review_status", "error_code", "metadata_json",
)
...
def migrate(db_path: Path, backup_path: Path | None = None) -> dict[str, str | int]:
    ...
    try:
        ...
        existing_table = connection.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='document_enrichments'"
        ).fetchone()
        if existing_table:
            raise RuntimeError("PREEXISTING_DOCUMENT_ENRICHMENTS_TABLE")
        migration_sql = MIGRATION.read_text(encoding="utf-8")
        connection.executescript("BEGIN IMMEDIATE;\n" + migration_sql)
        actual_columns = tuple(row[1] for row in connection.execute("PRAGMA table_info(document_enrichments)"))
        if actual_columns != EXPECTED_COLUMNS:
            raise RuntimeError("DOCUMENT_ENRICHMENTS_COLUMN_MISMATCH")
