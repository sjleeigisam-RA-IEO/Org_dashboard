...
export function ArchivedDetailDrawer({ result, onClose }: Props) {
  ...
  return <div className="drawer-layer" onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
    <section className="detail-drawer entity-detail-drawer" role="dialog" aria-modal="true" aria-label="로컬 보관 상세">
      <header className="drawer-header"><div><p className="eyebrow">LOCAL FULL ARCHIVE</p><h2>{result.title}</h2><p>Turso compact index · 상세 원본은 로컬 authority에 보관</p></div><button type="button" className="icon-button" aria-label="상세 닫기" onClick={onClose}>×</button></header>
      <div className="drawer-body entity-detail-body">
        <section className="domain-narrative"><p className="eyebrow">ARCHIVE STATUS</p><h3>비활성 상세 보관</h3><p>이 항목은 현재 active serving 범위에서 종료됐습니다. 검색용 최소 메타데이터는 Turso에 남고, 전체 evidence와 lineage는 checksum으로 고정된 로컬 SQLite snapshot에서 복원합니다.</p></section>
